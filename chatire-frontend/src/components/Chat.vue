    ...
    <div class="row">
      <div class="col-sm-6 offset-3">
        ...
        <div v-else-if="!loading && !sessionStarted">
          <h3 class="text-center">HALO {{ username }}!</h3>
          <br />
          <p class="text-center">
            Click tombol di bawah untuk memulai chat
          </p>
          <br />
          <button @click="startChatSession" class="btn btn-primary btn-lg btn-block">Mulai Chat</button>
        </div>
...
export default {
  ...
  created () {
    ...
    // Refresh JWT setiap detik contoh: 240 detik (4 menit)
  ...
  methods: {
    startChatSession () {
      $.post('http://localhost:8000/api/chats/', (data) => {
        // alert("sesi baru sudah di buat")
        console.log('sesi baru sudah di buat')
